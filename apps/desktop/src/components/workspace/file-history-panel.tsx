...
export function FileHistoryPanel({
  ...
  const restoreFile = useHistoryStore((s) => s.restoreFile);
  const addLabel = useHistoryStore((s) => s.addLabel);
  const removeLabel = useHistoryStore((s) => s.removeLabel);
  const openProject = useDocumentStore((s) => s.openProject);

  // Filter snapshots that involve this file
  const fileSnapshots = useMemo(() => {
    return snapshots.filter((snap) =>
      snap.changed_files.some((f) => f === filePath || f.endsWith(filePath)),
    );
  }, [snapshots, filePath]);

  // Compute linear history (skip restore jumps)
  ...
  useEffect(() => {
    ...
    loadSnapshots(projectRoot).catch(console.error);
  ...
  const handleRestore = useCallback(
    async (snapshotId: string) => {
      if (!projectRoot) return;
      useHistoryStore.getState().stopReview();
      await restoreFile(projectRoot, snapshotId, filePath);
      await openProject(projectRoot);
      await loadSnapshots(projectRoot);
    },
    [projectRoot, restoreFile, openProject, loadSnapshots, filePath],
  );
